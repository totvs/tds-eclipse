...
public class RefreshConsoleLog10Minutes extends ServerMonitorHandler {
	...
	public final Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		job = new RefreshServerConsoleLogJob("Refresh 10 minutes", 600000); //$NON-NLS-1$
		job.schedule();
		isEnabled = false;
		return null;
	}
	...
	private void sendNotification() {
		...
		INotification notification = notificationService.createNotification(
				INotification.EventType.CONSOLE_REFRESH_10_MIN, this);
	...
	private void hookNotification() {
		...
		NotificationHandler eventHandlerStateConnection = new NotificationHandler() {
			...
			public void receive(final INotification notification) {
				if (notification.getEventType() == INotification.EventType.CONSOLE_DEL_ITEM
						|| notification.getEventType() == INotification.EventType.CONSOLE_REFRESH_5_MIN
		...
		notificationService.subscribe(INotification.EventType.CONSOLE_REFRESH_5_MIN,
				eventHandlerStateConnection);
