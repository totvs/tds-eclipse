 ...
 */
public abstract class ProcessPatchJob extends Job {
	 ...
	 */
	public ProcessPatchJob(final String name, final ApplyPatchAttributes applyPatchAttributes) {
		super(name);
	...
	protected IStatus doValidatePatchs(final IProgressMonitor monitor) throws Exception {
		...
		for (final ApplyPatchFileReturn applyPatchFileReturn : applyPatchFilesReturn) {
			...
			final List<URI> patchList = new ArrayList<URI>();
			patchList.add(serverPatch.toUri());

			final ValidationPatchReturn vpr = appServer.validPatch(environment, patchList,
	...
	protected IStatus doApplyPatches(final IProgressMonitor monitor) throws Exception {
		...
		for (final ApplyPatchFileReturn applyPatchFileReturn : applyPatchFilesReturn) {
			...
			final ApplyPatchReturn ar = appServer.applyPatch(environment, patchFilename.toFile().toString(),
		...
		monitor.subTask("Limpando arquivos de trabalho");
		appServer.cleanUp(environment, CleanUpType.CLEANUP_PATCH);
		//
		return status;
	}
	...
	private ValidationPatchReturn validatePackageInServer(final IProgressMonitor monitor,
		...
		final List<URI> patchList = new ArrayList<URI>();
		patchList.add(new Path(applyPatchFileReturn.getPatchFullPath()).toFile().toURI());

		final ValidationPatchReturn vpr = appServer.validPatch(environment, patchList, applyPatchFileReturn.isLocal());
		applyPatchFileReturn.setValidationMessage(vpr.getReturnMessage());
		applyPatchFileReturn.setOldPrograms(vpr.getOldPrograms());
